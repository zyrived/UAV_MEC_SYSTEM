...
def evaluate_solution(sol, pop):
    ...
    first_node, last_node = sol[0], sol[-1]
    total_distance = np.linalg.norm(np.array(pop[first_node]) - np.array(pop[last_node]))
...
def apply_operator(sol, operator):
    ...
    temp = sol.copy()
    if operator == 'Swap' and len(temp) > 1 :
    ...
    elif operator == 'Reversion' and len(temp) > 1 :
...
def tabu_search_algorithm(C, MaxIT, POP):
    ...
    data = [list(item) if isinstance(item, np.ndarray) else item for item in Q]
    unique_data = [list(set(sublist)) if isinstance(sublist, list) else sublist for sublist in data]
    return unique_data
