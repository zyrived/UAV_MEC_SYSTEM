...
class SPDeploymentUpdater:
    def __init__(self, initial_population, num_generations=100, F=0.5, CR=0.7):
        # 初始化类，设置算法参数和目标函数
    ...
    def de_rand_1(self, a, b, c):
        # DE/rand/1 操作，生成新个体
        vector = [a[j] + self.F * (b[j] - c[j]) for j in range(len(a))]

        for i in [0, 1]:
            if abs(vector[i]) > 1000:
                vector[i] = 1000 * random.random()
            else :
    ...
    def binomial_crossover(self, target_vector, candidate_vector):
        ...
        for j in range(len(target_vector)):
            if random.random() < self.CR or j == random.randint(0, len(target_vector) - 1):
                candidate.append(abs(candidate_vector[j]))
            else:
                candidate.append(abs(target_vector[j]))
        return candidate
...
def evoluationary_operators_DE(initial_population):
    sp_updater = SPDeploymentUpdater(initial_population)
...
if __name__ == "__main__":
    # 定义目标函数
    # 初始化种群
    initial_population = [(374.5, 950.7, 100), (732.0, 598.7, 100), (156.0, 156.0, 100), (58.1, 866.2, 100),
    ...
    pop_updater = evoluationary_operators_DE(initial_population)
